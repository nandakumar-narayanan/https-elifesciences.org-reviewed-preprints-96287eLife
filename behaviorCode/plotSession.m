function SessionAnSt = plotSession(TrialAnSt, savefolder_name, no_plot)
if nargin < 3
    no_plot = false;
end
...
function rasterplot(TrialAnSt, animal)
    trialWithDataIdx = find(~cellfun('isempty', {TrialAnSt.(animal)}));
    numOfDay = numel(trialWithDataIdx);
    ...
    for i_day = 1:numOfDay
        ...
        colormap = [.8 .05 .05; .05 .05 .8; 1 0 0; 0 0 1];
        lwidth = [1 1 2 2];
        for i_trial_type = 1:2
            if i_trial_type == 1
                trialTypeIdx = [trialData.programmedDuration] == 18000;
                subplot(numplot_y,3*numplot_x,subplot_idx(1):subplot_idx(1)+1)
            elseif i_trial_type == 2
                trialTypeIdx = [trialData.programmedDuration] == 6000;
                subplot(numplot_y,3*numplot_x,subplot_idx(2))
            end
