...
%     pdf_switch_depart = zeros(numel(centers)-1,size(uni_ids,1));
    ...
    for uniNameIdx = 1:size(uni_ids,1)
        ...
        pooled_short_rsp = []; pooled_long_rsp = [];pooled_firstPoke = []; totFirstPoke = [];
        ...
        for groupMemberIdx = 1:length(groupMember)
            ...
            if numel([tmp_struct.SwitchDepart]) < 2 % this restriction was only applied to opto data?
                continue
            end
            ...
            for i_type = 1:length(uni_type)
                ...
                if exist('tmp_struct(trial_type_idx & switch_idx).firstPoke')
                se = [tmp_struct(trial_type_idx & switch_idx).firstPoke];
                pooled_firstPoke(1:numel(se),i_type) = se;
                else firstPoke = NaN; 
                end
            ...
            totFirstPoke = [totFirstPoke; pooled_firstPoke];
        ...
        for i_var = 1:2
            ...
            for i_type = 1:num_type
                ...
                [f1,~] = ksdensity(nonzeros(st(:,i_type)),centers(2:end),'Bandwidth',1.0);
        ...
        totFirstPoke(totFirstPoke==0) = NaN;
        ...
        GroupAnSt.(groupFields{groupIdx}).firstPoke{uniNameIdx} = totFirstPoke; 
