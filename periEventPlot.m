 function varargout = periEventPlot(periEvtSPK, interval, params)
...
for condIdx = 1:numCond
    if ~isempty(periEvtSPK{condIdx})
        ...
        if ~p.addCI
            hold on; 
            data = gksmooth(periEvtSPK{condIdx}, interval, kernelWin*1.5);
%              data = zscore(data);
            plot(interval.*timeScale, data,'LineWidth',5, 'Color', p.cmap(condIdx,:))
        else
            [data, ci] = gksmooth(periEvtSPK{condIdx}, interval, kernelWin);
%             data = zscore(data);
            plotband(interval.*timeScale, data, ci, p.cmap(condIdx,:))
        end
    end
end
