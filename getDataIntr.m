...
% 3/15/2023 : fix group, date range
...
elseif isfolder(folder_name)
    ...
    fileList = regexpi({tmp_fileList.name},'(^!\d{4}-\d{2}-\d{2}|^\d{4}-\d{2}-\d{2}.txt)','match');
    fileList = [fileList{:}];
...
if iscell(dateRange) && ~isempty(dateRange) % date range is requested
    ...
    if length(dateRange) < 3 % 1 or 2 is considered as range
        ...
        % TODO check date data type automatically 
...
% remove data with no subject name and empty name
mpcParsed = mpcParsed(~strcmp('0',{mpcParsed.Subject}));
mpcParsed = mpcParsed(cellfun(@(x) ~isempty(x),{mpcParsed.Subject}));
% fix Animal ID
